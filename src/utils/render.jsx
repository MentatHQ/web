...
<html>
  <head>
    ...
    <link rel="stylesheet" href="${process.env.ENV === 'local' ? 'http://localhost:6148' : ''}/index.css" />
    <link rel="stylesheet" href="${process.env.ENV === 'local' ? 'http://localhost:6148' : ''}/fonts/main.css" />
    <link rel="stylesheet" href="${process.env.ENV === 'local' ? 'http://localhost:6148' : ''}/fonts/icons.css?v=0.0.2" />
  </head>
  <body>
    <div id="root">${renderToStaticMarkup(component)}</div>
    <script>window.INITIAL_STATE = ${JSON.stringify(data)};</script>
    <script src="${process.env.ENV === 'local' ? 'http://localhost:6148' : ''}/bundle.js" async></script>
  </body>
</html>`
);
